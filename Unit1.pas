...
type
  TCalculateProFire = class(TForm)
    ...
    ADOTable1кварталы: TWideStringField;
    ADOTable1лесничества: TIntegerField;
    ADOTable1лесхоз: TIntegerField;
    ...
    ADOTable2кварталы: TWideStringField;
    ADOTable2лесничества: TIntegerField;
    ADOTable2лесхоз: TIntegerField;
...
var
  ...
  avz: integer;
begin
  // Ежедневная оценка пожарной опасности
  // stringgrid1:=RashetPO(mySQLDatabase1,stringgrid1,ist,2010,0);

  // Определения последний даты ежедневной оценки
  b := true;
  ...
  // Определения последний даты прогноза
  for i := 1 to str.RowCount do
    if str.cells[0, i] = dsr then
      indpr := i - 1;

  // Определения с какого дня начинается расчет
  if indpr >= indfc then
    ind := indfc
  else
    ind := indpr;

  // Показаль засухи текщего дня
  lpz := strtofloat(str.cells[4, ind]);
  pz := trunc(strtofloat(str.cells[5, ind]));

  // Определения месяца у каждого дня прогноза
  ...
  // Расчет ЛПЗ по прогностическим уравнениям
...
begin
  ...
  // Почему мокрых дней нет
  if status = 2 then
    status := 1;
  // загрузка прогностических уравнений в зависимости от статуса статус 1-сухие 2 мокрые
...
begin
  f := 0;
  if str = 'Небольшой дождь' then
    f := 2;
  if str = 'Небольшие осадки' then
    f := 2;
  if str = 'Преимущественно без осадков' then
    f := 2;
  if str = 'Дождь' then
    f := 6;
  if str = 'Без осадков' then
    f := 1;
  if str = 'Временами осадки' then
    f := 5;
  if str = 'Кратковременный дождь' then
    f := 4;
  if str = 'Небольшой снег' then
    f := 2;
  if str = 'Cнег' then
    f := 6;
  formalnest := f;
...
// kp-комплексный показатель засухи
// k-месяц пожароопасного сезона
// avz - номер авиаотделения
var
  p: boolean;
  kl: integer;
begin
...
begin
  ...
  begin
    ...
    // Расчет индекс продолжительности тепла HWDD
    ...
    begin
      ...
      begin
        ...
        begin
          if ((temp_do + temp_posle) / 2 < tros) then
            showmessage('Обшибка данных ' + mySQLQuery1.Fields[0].Text)
          else
      ...
      begin
        case metod of
          ...
          showmessage('Вызов несуществующей методики');
        end;
...
begin
  ...
  begin
    ...
    kvartals[3, lll] := ADOTable1.FieldByName('лесхоз').AsLargeInt;
    kvartals[4, lll] := ADOTable1.FieldByName('лесничества').AsLargeInt;
    kvartals[5, lll] := ADOTable1.FieldByName('кварталы').AsLargeInt;
...
begin
  ...
  begin
    id_les := ADOTable2.FieldByName('лесничества').AsLargeInt;
    id_kvar := ADOTable2.FieldByName('кварталы').AsLargeInt;
    fires := 0;
...
begin
  // Биробиджан
  masXYZ[0, 1] := 21; // Код метеостанции
  masXYZ[1, 1] := 22566013; // Координата х
  masXYZ[2, 1] := 5406183; // Координата y
  masXYZ[3, 1] := 0; // Значение узлов интерполяции ЛПЗ i дня
  masXYZ[4, 1] := 0; // Суточный объем осадков  i дня
  // Смидовичи
  masXYZ[0, 2] := 27;
  masXYZ[1, 2] := 22633130;
  masXYZ[2, 2] := 5387003;
  masXYZ[3, 2] := 0; // Значение узлов интерполяции ЛПЗ i дня
  masXYZ[4, 2] := 0; // Суточный объем осадков  i дня
  // Ленинское
  masXYZ[0, 3] := 26;
  masXYZ[1, 3] := 22543941;
  masXYZ[2, 3] := 5312325;
  masXYZ[3, 3] := 0; // Значение узлов интерполяции ЛПЗ i дня
  masXYZ[4, 3] := 0; // Суточный объем осадков  i дня
  // Екатерино-Никольское
  masXYZ[0, 4] := 28;
  masXYZ[1, 4] := 22422277;
  masXYZ[2, 4] := 5292308;
  masXYZ[3, 4] := 0; // Значение узлов интерполяции ЛПЗ i дня
  masXYZ[4, 4] := 0; // Суточный объем осадков  i дня
  // Облучье
  masXYZ[0, 5] := 29;
  masXYZ[1, 5] := 224305555;
  masXYZ[2, 5] := 5431406;
  masXYZ[3, 5] := 0; // Значение узлов интерполяции ЛПЗ i дня
  masXYZ[4, 5] := 0; // Суточный объем осадков  i дня
  // Сутур
  masXYZ[0, 6] := 17;
  masXYZ[1, 6] := 22545300;
  masXYZ[2, 6] := 5542000;
  masXYZ[3, 6] := 0; // Значение узлов интерполяции ЛПЗ i дня
  masXYZ[4, 6] := 0; // Суточный объем осадков  i дня
  // Кукан
  masXYZ[0, 7] := 18;
  masXYZ[1, 7] := 22611400;
  masXYZ[2, 7] := 5453800;
  masXYZ[3, 7] := 0; // Значение узлов интерполяции ЛПЗ i дня
  masXYZ[4, 7] := 0; // Суточный объем осадков  i дня
  // Кур
  masXYZ[0, 8] := 19;
  masXYZ[1, 8] := 22664300;
  masXYZ[2, 8] := 5443200;
  masXYZ[3, 8] := 0; // Значение узлов интерполяции ЛПЗ i дня
  masXYZ[4, 8] := 0; // Суточный объем осадков  i дня

  // Троицкое
  masXYZ[0, 9] := 20;
  masXYZ[1, 9] := 226716863;
  masXYZ[2, 9] := 5477602;
  masXYZ[3, 9] := 0; // Значение узлов интерполяции ЛПЗ i дня
  masXYZ[4, 9] := 0; // Суточный объем осадков  i дня

  // Хабаровск
  masXYZ[0, 10] := 22;
  masXYZ[1, 10] := 22725000;
  masXYZ[2, 10] := 5384600;
  masXYZ[3, 10] := 0; // Значение узлов интерполяции ЛПЗ i дня
  masXYZ[4, 10] := 0; // Суточный объем осадков  i дня
  ...
  begin
    begin
      ...
      begin
        ...
        // leshoz:= ADOTable1.FieldByName('лесхоз').AsLargeInt;
        lesnich := kvartals[4, lll];
        // lesnich:=ADOTable1.FieldByName('лесничества').AsLargeInt;
        kvartal := kvartals[5, lll];
        // kvartal:=ADOTable1.FieldByName('кварталы').AsLargeInt;
        ist := kvartals[6, lll];
        ...
        // Расчет возгораемости по погодным условиям
        ...
        begin
          ...
          case RadioGroup1.ItemIndex of
            ...
            1:
              if (lesnich <> 0) then
                  ...
                  4:
                    begin
                      ...
                      begin
                        ...
                        poj_t0 := Poj_fact(mySQL_ОТЕ_year1, strtodate(datek),
                          lesnich, kvartal, strtoint(ey1.Text),
                          strtoint(ey2.Text));
                    end;
                ...
                poj_t0 := Poj_fact(mySQL_ОТЕ_year1, strtodate(datek), lesnich,
          ...
          // Не учитываем ОТЕ не лесного фонда

          if poj_x > 0 then

            if kvartals[17, lll] < 9000 then
              Memo1.Lines.Add(datetostr(incday(strtodate(datek),
                RadioGroup3.ItemIndex)) + ';Лесхоз;' + inttostr(leshoz) +
                ';Лесничество; ' + inttostr(lesnich) + ';Квартал; ' +
                inttostr(kvartal) + '; КППО; ' + floattostr(prgkl) + '; R; ' +
...

        {
          ...
          begin
          if kvartals[17,lll]<9000 then
          memo1.Lines.Add(datek+';Лесхоз;'+inttostr(leshoz)+';Лесничество; '+inttostr(lesnich)+';Квартал; '+inttostr(kvartal)
          +'; КППО; '+floattostr(prgkl)+'; Расстояние; '+floattostr(kvartals[17,lll])+';F(N); '+floattostr(poj_t0)
          +';F(N/B); '+floattostr(countn)+';F(C);'+floattostr(k1)
          +';F(B); '+floattostr(kvartals[16,lll]/100))

          else
          memo1.Lines.Add(datek+';Лесхоз;'+inttostr(leshoz)+';Лесничество; '+inttostr(lesnich)+';Квартал; '+inttostr(kvartal)
          +'; КППО; '+floattostr(prgkl)+'; Расстояние; '+floattostr(kvartals[18,lll])+';F(D); '+floattostr(poj_t0)
        ...
        { poj_2:=poj(mySQLQuery2,strtodate(datetostr(incday(strtodate(datek),2))),lesnich,kvartal);
          ...
          memo1.Lines.Add(datek+'; лес-во; '+inttostr(lesnich)+';квартал; '+inttostr(kvartal)
          +'; antropogen pos_а; '+floattostr(p_a)
          +'; antropogen uch; '+floattostr(poj_t0)+';Pреал2; '+floattostr(p2)+'; Pкрит2; '+floattostr(ok2)
          +'; Класс пир; '+floattostr(prgkl))
        ...
        { poj_3:=poj(mySQLQuery2,strtodate(datetostr(incday(strtodate(datek),3))),lesnich,kvartal);
          ...
          memo1.Lines.Add(datek+'; лес-во; '+inttostr(lesnich)+';квартал; '+inttostr(kvartal)
          +'; antropogen pos_а; '+floattostr(p_a)
          +'; antropogen uch; '+floattostr(poj_t0)+';Pреал3; '+floattostr(p3)+'; Pкрит3; '+floattostr(ok3)
          +'; Класс пир; '+floattostr(prgkl))
...
begin
  ...
  // 'База данных лесхоз.mdb;Mode=Share Deny None;Persist Security Info=False;Jet OLEDB:System database="";'+
...
begin
  ...
  // Поиск суточного объема осадков по  метеостанции
  for i := 1 to 10 do
...
begin
  ...
  begin
    ...
    v := v + (1 / mas[i]); // нашли весовую функцию
  end;
  for j := 3 to 6 do
  begin
    res[j] := 0;
    for i := 1 to 10 do
      if mas[i] < 3000 then
        res[j] := masXYZ[2 + j, i]
      else // учет значений по ближайщей метостанции 30 км
        res[j] := res[j] + (masXYZ[2 + j, i] / (mas[i] * v));
    // расчет значения в точке интерполирования
  end;
  R_LGKP := res;
end;
...
begin
  ...
  // Поиск прогнозируемой интенсивности по ближайщей  метеостанции
